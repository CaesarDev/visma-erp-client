...
------------ | ------------- | ------------- | -------------
**total_cost** | **float** | The top part &amp;gt; Total cost &amp;gt; The total cost of the inventory items received. The system automatically calculates this value as a sum of all extended cost values of all received inventory items listed in the Transaction Details tab of this form. | [optional]
**control_cost** | **float** | The top part &amp;gt; Control cost &amp;gt; The manually entered cost of stock items listed in the adjustment. Control cost is available only if the Validate document totals on Entry option is selected on the Inventory preferences window. The user enters this value manually to confirm the total cost as calculated automatically for the document. If the Control Cost and Total Cost values do not match, the system generates a warning message and the document cannot be saved. | [optional]
**adjusment_lines** | [**\OpenAPI\Client\Model\InventoryAdjustmentLineDto[]**](InventoryAdjustmentLineDto.md) | Transaction details tab &amp;gt; The inventory adjusment lines | [optional]
...
**date** | **\DateTime** | The date when the inventory document was created. All transactions included in this document will have this transaction date. | [optional]
...
**total_quantity** | **float** | The total quantity of inventory items, which the system calculates automatically, for the document. | [optional]
**control_quantity** | **float** | The manually entered quantity of inventory items. Control Qty. is available only if the Validate Document Totals on Entry option is selected on the Inventory Preferences form. If the Control Qty. and Total Qty.values do not match, the system generates a warning message and the issue cannot be saved. | [optional]
...
**last_modified_date_time** | **\DateTime** | The date when the document was last modified. | [optional]
**branch_number** | [**\OpenAPI\Client\Model\BranchNumberInInventoryAdjustmentDto**](BranchNumberInInventoryAdjustmentDto.md) |  | [optional]
...
**timestamp** | **string** | Timestamp of the inventory document | [optional]
