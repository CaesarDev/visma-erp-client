...
**control_cost** | **float** | The total cost of the inventory items to be received. Control Cost is available only if the Validate Document Totals on Entry option is selected on the Inventory Preferences form. The user enters this value manually to confirm the total cost as calculated automatically for the document. If the Control Cost and Total Cost values do not match, the system generates a warning message and the document cannot be saved. | [optional]
...
**receipt_lines** | [**\OpenAPI\Client\Model\InventoryReceiptLineDto[]**](InventoryReceiptLineDto.md) | The inventory receipt lines | [optional]
...
**branch_number** | [**\OpenAPI\Client\Model\BranchNumberInInventoryReceiptDto**](BranchNumberInInventoryReceiptDto.md) |  | [optional]
