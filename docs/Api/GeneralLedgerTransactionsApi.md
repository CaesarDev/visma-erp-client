# OpenAPI\Client\GeneralLedgerTransactionsApi
...
| ------------- | ------------- | ------------- |
| [**generalLedgerTransactionsGetAll()**](GeneralLedgerTransactionsApi.md#generalLedgerTransactionsGetAll) | **GET** /v1/GeneralLedgerTransactions | Get a range of General Ledger Transactions - ScreenId&#x3D;GL404000  Please use a page size lower or equal to the allowed max page size which is returned under metadata.   If pagesize is greater than the max page size, it will be limited to max page size. |


## `generalLedgerTransactionsGetAll()`

```php
generalLedgerTransactionsGetAll($ledger, $branch, $from_period, $to_period, $account, $subaccount_id, $from_date, $to_date, $include_unposted, $include_unreleased, $skip_records, $number_to_read, $last_modified_date_time, $last_modified_date_time_condition, $expand_account_info, $expand_branch_info, $include_transaction_balance, $page_number, $page_size, $erp_api_background): \OpenAPI\Client\Model\GeneralLedgerTransactionDetailsDto[]
```
...
$apiInstance = new OpenAPI\Client\Api\GeneralLedgerTransactionsApi(
...
$from_period = 'from_period_example'; // string | The financial period that begins the date range of the batches you want to view. Format YYYYPP
$to_period = 'to_period_example'; // string | The financial period that ends the date range of the batches you want to view. Format YYYYPP
...
$expand_account_info = True; // bool | By default, if no value is provided, False and the account information will include only Number, Description, Type and GlConsolAccountCD.  If True, each transaction returned will include extended information about account.
$expand_branch_info = True; // bool | By default, if no value is provided, False and branch information will include only branch number.  If True, each transaction returned will include extended information about Branch (number and name).
$include_transaction_balance = True; // bool | By default, if no value is provided, False and the transactions returned will not include their balances (fields BegBalance, EndingBalance, CurrBegBalance, CurrEndingBalance)  If True, each transaction returned will include its balance.
...
try {
    $result = $apiInstance->generalLedgerTransactionsGetAll($ledger, $branch, $from_period, $to_period, $account, $subaccount_id, $from_date, $to_date, $include_unposted, $include_unreleased, $skip_records, $number_to_read, $last_modified_date_time, $last_modified_date_time_condition, $expand_account_info, $expand_branch_info, $include_transaction_balance, $page_number, $page_size, $erp_api_background);
    print_r($result);
} catch (Exception $e) {
...
[**\OpenAPI\Client\Model\GeneralLedgerTransactionDetailsDto[]**](../Model/GeneralLedgerTransactionDetailsDto.md)
