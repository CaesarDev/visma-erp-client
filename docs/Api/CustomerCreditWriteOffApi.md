# OpenAPI\Client\CustomerCreditWriteOffApi
...
$apiInstance = new OpenAPI\Client\Api\CustomerCreditWriteOffApi(
...
[**\OpenAPI\Client\Model\CustomerCreditWriteOffDto[]**](../Model/CustomerCreditWriteOffDto.md)
...
```php
customerCreditWriteOffGetBycreditWriteOffNumber($credit_write_off_number, $erp_api_background): \OpenAPI\Client\Model\CustomerCreditWriteOffDto
```

Get a specific Customer Credit WriteOff

Data for Customer Credit WriteOff <br></br>              The response headers include an ETag after a successful GET operation.
...
try {
    $result = $apiInstance->customerCreditWriteOffGetBycreditWriteOffNumber($credit_write_off_number, $erp_api_background);
    print_r($result);
} catch (Exception $e) {
